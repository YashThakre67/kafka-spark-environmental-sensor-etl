...
def process_micro_batch(
    ...
    """
    Processes one Spark micro-batch in distributed mode.
    ...
    Kafka streaming DataFrame
        ...
        -> distributed file sink output
    """
    if batch_df.rdd.isEmpty():
        print(f"Batch {batch_id}: empty batch")
        return

    print(f"Processing distributed batch {batch_id}")
    ...
    final_output_rdd = joined_rdd.map(
        lambda record: {
            **record,
            ...
            "execution_mode": "distributed_4_workers",
...
def main():
    parser = argparse.ArgumentParser(
        description="Distributed Kafka-Spark environmental sensor ETL pipeline."
    )

    parser.add_argument(
        "--spark-master",
        default="spark://spark-master:7077",
        help="Spark master URL for distributed execution.",
    )
    ...
    parser.add_argument(
        "--output-path",
        default="/opt/spark-output/distributed",
        help="Output path for distributed processed results.",
    )

    parser.add_argument(
        "--checkpoint-path",
        default="/tmp/spark-checkpoints/environmental-etl-distributed",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of Spark partitions used for distributed processing.",
    )
    ...
    spark = (
        ...
        .appName("Kafka-Spark Environmental Sensor ETL - Distributed")
    ...
    print("Distributed environmental sensor ETL pipeline started.")
